...
class CutOrder:
    ...
    def __init__(self, id, due_date, priority, vars_dict):
        ...
        # Can be changed to self._priority = priority if desired / needed
   ...
   def OnStart(self, event):
      ...
      while i < len(order_list):
          ...
          while sublist:
              ...
              # As of now, above assumes any empty table will be large enough to accommodate any one order
...
################
   ...
   def OnHelp(self, event):
      wx.MessageBox("This application is used for providing a sorted list of cut orders in such a way that provides " +
                    "a solution with the least cutter down time and longest cut time while also prioritizing orders with" +
                    "the earliest due date orders with the highest priority code.\n" +
                    "\n" +
                    "The current format for input of order data is an Excel file with two sheets, the first sheet with" +
                    "order-specific information and the second for constants used in calculations involved in calculating" +
                    "the total time, cut time, and spread time.  Examples of this can be found in the 'Dummy Data 1.xlsx' and " +
                    "'Dummy Data 2.xlsx' files, with the results of running this program on these files - 'Sorted Orders 1.xlsx'" +
                    "and 'Sorted Orders 2.xlsx', respectively - also included.  Order data input MUST be done this way, unless the" +
                    "program has been edited.\n" +
                    "\n" +
                    "Specific information can be saved to be loaded on startup.  Other information can be" +
                    " used, but will not be retained unless the configuration being used is saved, in which case that information" +
                    " will be what is loaded on startup going forward.\n" +
                    "\n" +
                    "Always check to ensure that the location and names of the files / directories being used are accurate" +
                    " and up to date.  Inaccurate file or directory names or inaccurate locations will cause the program to " +
                    "be unable to run.\n" +
                    "\n" +
                    "For more information see the README.txt file.", "Help", wx.ICON_INFORMATION)
